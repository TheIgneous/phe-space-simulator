...
const initialDataset = rawDataset as SimulationDataset;
const INITIAL_SELECTION: Selection = { term: "T1a", week: "A", day: 0, time: 8 * 60 };

export function App() {
  const [view, setView] = useState<"simulator" | "plan">("simulator");
  ...
  const units = useMemo(() => [...new Set(draftAssignments.map((assignment) => assignment.activity))].sort(), [draftAssignments]);
  const staff = useMemo(
  ...
  return (
    <main className="app-frame">
      <nav className="view-tabs" aria-label="Viewer pages">
        <button type="button" className={view === "simulator" ? "active" : ""} aria-current={view === "simulator" ? "page" : undefined} onClick={() => setView("simulator")}>Simulator</button>
        <button type="button" className={view === "plan" ? "active" : ""} aria-current={view === "plan" ? "page" : undefined} onClick={() => setView("plan")}>Plan</button>
      </nav>
      {view === "plan" ? (
        <PlanPage
